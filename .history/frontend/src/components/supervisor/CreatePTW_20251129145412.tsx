...
export function CreatePTW({ onBack, onSuccess }: CreatePTWProps) {
  ...
  useEffect(() => {
    if (formData.categories.length > 0) {
      loadAllChecklistQuestions(formData.categories);
    }
  }, [formData.categories]);
  ...
  const handleNext = () => {
    ...
    if (currentStep === 1) {
      ...
      if (!formData.site_id || !formData.location || !formData.workDescription) {
        alert('Please fill in all required fields');
        return;
      }
    }

    if (currentStep === 6 && requiresSiteLeaderApproval) {
      if (!approvers.areaManager || !approvers.safetyOfficer || !approvers.siteLeader) {
        alert('Please select all required approvers including Site Leader');
        return;
      }
    } else if (currentStep === 6) {
      if (!approvers.areaManager || !approvers.safetyOfficer) {
        alert('Please select Area Manager and Safety Officer');
        return;
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const permitData = {
        ...
        issue_department: formData.issueDepartment, // Changed from vendor_id
        ...
        // NEW: Approvers
        area_manager_id: approvers.areaManager,
        safety_officer_id: approvers.safetyOfficer,
        site_leader_id: requiresSiteLeaderApproval ? approvers.siteLeader : null,
      };
              ...
              <div>
                ...
                {sites.length === 0 && (
                  <p className="mt-1 text-xs text-red-600">
                    No sites loaded. Please refresh the page.
                  </p>
                )}
              </div>
              <div>
        ...
        {currentStep === 5 && (
          <div className="space-y-6">
            ...
            <p className="text-sm text-slate-600">
              Complete the checklist for all selected permit types
            </p>
            ...
            <div className="p-6 border rounded-lg border-slate-200">
              {formData.categories.length > 0 ? (
                    ...
                    return (
                      <div key={category} className="pb-6 border-b border-slate-200 last:border-0">
                        <h3 className="mb-4 text-lg font-semibold text-slate-900">
                          {category.replace('_', ' ')} Requirements
                        </h3>
                        {categoryQuestions.length > 0 ? (
                          ...
                          <p className="text-sm text-slate-500">No checklist questions available for this permit type</p>
                        )}
                      </div>
                    );
                  })}
                ...
                <p className="text-center text-slate-500">Please select permit categories first</p>
              )}
            </div>
          </div>
        )}
        ...
        {currentStep === 6 && (
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              {/* Area Manager */}
              <div>
                <Label htmlFor="areaManager">Area Manager * (Always Required)</Label>
                <Select
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="0">-- Select Area Manager --</SelectItem>
              ...
              {/* Safety Officer */}
              <div>
                <Label htmlFor="safetyOfficer">Safety Officer * (Always Required)</Label>
                <Select
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="0">-- Select Safety Officer --</SelectItem>
              ...
              {requiresSiteLeaderApproval && (
                <div className="p-4 border-2 border-red-200 rounded-lg bg-red-50">
                  <Label htmlFor="siteLeader" className="text-red-900">
                    Site Leader * (Required for High-Risk Permits)
                  </Label>
                  <Select
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="0">-- Select Site Leader --</SelectItem>
            ...
            <div className="p-4 rounded-lg bg-slate-50">
              <h3 className="mb-3 font-semibold text-slate-900">Approval Workflow Summary</h3>
              <div className="space-y-2 text-sm">
                <div className="flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full ${approvers.areaManager ? 'bg-green-500' : 'bg-slate-300'}`} />
                  <span className="text-slate-700">
                    Area Manager: {areaManagers.find(m => m.id === approvers.areaManager)?.full_name || 'Not selected'}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full ${approvers.safetyOfficer ? 'bg-green-500' : 'bg-slate-300'}`} />
                  <span className="text-slate-700">
                    Safety Officer: {safetyOfficers.find(o => o.id === approvers.safetyOfficer)?.full_name || 'Not selected'}
                  </span>
                </div>
                {requiresSiteLeaderApproval && (
                  <div className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full ${approvers.siteLeader ? 'bg-green-500' : 'bg-slate-300'}`} />
                    <span className="text-slate-700">
                      Site Leader: {siteLeaders.find(l => l.id === approvers.siteLeader)?.full_name || 'Not selected'}
                    </span>
                  </div>
                )}
              </div>
