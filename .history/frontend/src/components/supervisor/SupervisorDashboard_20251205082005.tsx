...
// COMPLETE SUPERVISOR DASHBOARD WITH ALL FUNCTIONALITY
...
export default function SupervisorDashboard({ onNavigate }: SupervisorDashboardProps) {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const [initiated, approved, rejected, readyToStart, inProgress, closed] = await Promise.all([
  ...
  const PermitTable = ({ 
    ...
    emptyMessage: string; 
    showActions?: boolean;
    actionType?: 'final-submit' | 'start' | 'extend-close' | 'view' | 'none';
  }) => (
                        ...
                        {actionType === 'extend-close' && (
                          <>
                            <button
                              ...
                              className="flex items-center gap-1 px-3 py-1 text-xs font-medium text-white bg-orange-600 rounded hover:bg-orange-700"
                            >
  ...
  const totalPermits = initiatedPermits.length + approvedPermits.length + 
                       ...
                       inProgressPermits.length + closedPermits.length;
