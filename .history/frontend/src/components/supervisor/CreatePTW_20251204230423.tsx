...
const handleSubmit = async () => {
  try {
    ...
    // ✅ CRITICAL FIX: Map categories to permit_types
    // The form uses "categories" but backend expects "permit_types"
    const selectedPermitTypes = formData.categories.join(','); // Convert array to string

    if (!selectedPermitTypes) {
      alert('❌ Please select at least one permit type (Step 1)');
    ...
    if (!formData.areaManagerId && !formData.safetyOfficerId) {
      alert('❌ Please select at least one approver (Area Manager or Safety Officer)');
      setIsSubmitting(false);
      return;
    }
    ...
    const payload = {
      ...
      permit_types: selectedPermitTypes, // ✅ FIX: Send as string
      work_description: formData.workDescription || 'Not provided',
      work_location: formData.workLocation || 'Not specified',
      start_time: formData.startTime,
      end_time: formData.endTime,
      receiver_name: formData.receiverName || 'Not specified',
      receiver_contact: formData.receiverContact || null,
      permit_initiator: formData.permitInitiator || null,
      permit_initiator_contact: formData.permitInitiatorContact || null,
      issue_department: formData.issueDepartment || null,
      team_members: formData.teamMembers || [],
      hazard_ids: formData.hazardIds || [],
      ppe_ids: formData.ppeIds || [],
      control_measures: formData.controlMeasures || null,
      other_hazards: formData.otherHazards || null,
      checklist_responses: formData.checklistResponses || [],
      swms_file_url: formData.swmsFileUrl || null,
      swms_text: formData.swmsText || null,
      area_manager_id: formData.areaManagerId || null,
      safety_officer_id: formData.safetyOfficerId || null,
      site_leader_id: formData.siteLeaderId || null,
    };

    console.log('📤 Submitting PTW with payload:', payload);

    const response = await permitsAPI.create(payload);
    
    console.log('📥 Response:', response);

    if (response.success) {
      alert('✅ PTW created successfully and submitted for approval!');
      onBack();
    } else {
    ...
    alert('Error creating PTW: ' + (error.response?.data?.message || error.message));
  } finally {
    setIsSubmitting(false);
  }
};
