...
export default function SupervisorDashboard({ onNavigate }: SupervisorDashboardProps) {
  ...
  try {
    ...
    // ✅ NEW: Load separate permit categories
    const [
      initiatedRes,
      approvedRes,
      readyRes,
    ...
    // Combine all permits for overall list (or separate them into state variables)
    setAllPermits([...initiated, ...approved, ...ready, ...inProgress, ...closed]);
