      ...
      <div className="p-6 bg-white border rounded-xl border-slate-200">
        ...
        {currentStep === 4 && (
          <div className="space-y-6">
            ...
            {/* PPE Section with VISIBLE Icons */}
            <div>
              ...
              <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                {ppeItems.map((ppe) => (
                  <button
                    ...
                    className={`flex flex-col items-center gap-3 p-4 border-2 rounded-lg transition-all ${formData.selectedPPE.includes(ppe.id)
                        ? 'border-blue-500 bg-blue-50 shadow-md'
                        : 'border-slate-200 hover:border-slate-300 hover:bg-slate-50'
                      }`}
                  >
                    {/* PPE Icon - Now Visible */}
                    <div className={`${formData.selectedPPE.includes(ppe.id) ? 'text-blue-600' : 'text-slate-400'}`}>
                      <PPEIconComponent name={ppe.name} />
                    </div>

                    {/* PPE Name */}
                    <span className={`text-sm font-medium text-center ${formData.selectedPPE.includes(ppe.id) ? 'text-blue-900' : 'text-slate-700'
                      }`}>
                      {ppe.name}
                    </span>
