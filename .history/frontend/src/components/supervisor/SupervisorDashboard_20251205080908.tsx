    ...
    <div className="min-h-screen p-6 bg-gray-50">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="w-full">
          {/* Tab Buttons */}
          <div className="flex gap-2 p-1 mb-4 overflow-x-auto rounded-lg bg-slate-100">
            <button
              onClick={() => setActiveTab('initiated')}
              className={`flex-1 min-w-fit px-4 py-2 text-sm font-medium rounded-md transition-all ${
                activeTab === 'initiated'
                  ? 'bg-white text-slate-900 shadow'
                  : 'text-slate-600 hover:bg-slate-200'
              }`}
            >
              Initiated ({initiatedPermits.length})
            </button>
            <button
            ...
            >
              Ready ({readyToStartPermits.length})
            </button>
            <button
