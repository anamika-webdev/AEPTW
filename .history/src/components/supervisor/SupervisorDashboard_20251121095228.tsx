...
export function SupervisorDashboard({ onNavigate, onPTWSelect }: SupervisorDashboardProps) {
  ...
  // Handler to close a PTW
  const handleClosePTW = (ptwId: string, e: React.MouseEvent) => {
    e.stopPropagation(); // Prevent row click from triggering
    
    if (window.confirm('Are you sure you want to close this PTW? This action will mark the permit as closed.')) {
      // In production, this would make an API call to update the PTW status
      console.log(`Closing PTW: ${ptwId}`);
      alert('PTW Closed Successfully! This is a demo - in production, this would update the database.');
      // In a real application, you would:
      // 1. Call an API to update the PTW status to 'closed'
      // 2. Refresh the data or update the local state
    }
  };

  // Check if a PTW can be closed
  const canClosePTW = (status: string) => {
    // Allow closing for: completed, in-progress, approved statuses
    // Don't allow closing for: closed, rejected, expired, initiated, pending
    return ['completed', 'in-progress', 'approved'].includes(status);
  };
          ...
          <table className="w-full">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-sm text-slate-600">Location</th>
                <th className="px-6 py-4 text-left text-sm text-slate-600">Workers</th>
            ...
            <tbody className="divide-y divide-slate-200">
              {ptws.map((ptw) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 text-sm text-slate-600">{ptw.location}</td>
                  <td className="px-6 py-4 text-sm text-slate-600">{ptw.assignedWorkers.length}</td>
                  <td className="px-6 py-4 text-sm text-slate-600">{ptw.startDate}</td>
                  <td className="px-6 py-4">
  ...
  return (
    <div className="space-y-6">
      ...
      {/* PTW Extended - Comprehensive View WITH CLOSE OPTION */}
      <div className="bg-white rounded-xl border border-slate-200 p-4 md:p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-slate-900">PTW Extended - All Permits</h3>
            <p className="text-sm text-slate-500 mt-1">Comprehensive view of all PTW permits with status highlights and actions</p>
          </div>
                ...
                return (
                  ...
                  >
                    <td className="px-3 py-3">
                      <p className="text-sm text-slate-900">{ptw.ptwNumber}</p>
                    </td>
                    <td className="px-3 py-3">
                      <span className={`px-2 py-1 rounded text-xs ${getCategoryColor(ptw.category)}`}>
                        {ptw.category}
                    ...
                    <td className="px-3 py-3">
                      <p className="text-sm text-slate-700 max-w-xs truncate">{ptw.workDescription}</p>
                    </td>
                    ...
                    <td className="px-3 py-3">
                      {canClosePTW(ptw.status) && (
                        <Button
                          onClick={(e) => handleClosePTW(ptw.id, e)}
                          size="sm"
                          variant="outline"
                          className="text-xs gap-1 hover:bg-red-50 hover:text-red-700 hover:border-red-300"
                        >
          ...
          <div className="space-y-3">
            {myPTWs.map((ptw) => {
              ...
              return (
                <div
                  key={ptw.id}
                  className={`p-4 rounded-lg transition-all ${getCardColor()}`}
                >
                  <div 
                    onClick={() => onPTWSelect(ptw.id)}
                  ...
                  {canClosePTW(ptw.status) && (
                    <div className="mt-3 pt-3 border-t border-slate-200">
                      <Button
                        ...
                        size="sm"
                        variant="outline"
                        className="w-full text-xs gap-1 hover:bg-red-50 hover:text-red-700 hover:border-red-300"
                      >
                        <XCircle className="w-3 h-3" />
                        Close PTW
                      </Button>
