...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    if (lastPermit.length > 0) {
      const lastNumber = parseInt(lastPermit[0].permit_serial.split('-')[1]);
    ...
    const [permitResult] = await connection.query(`
      INSERT INTO permits (
        permit_serial, site_id, permit_type, work_description, work_location,
        start_time, end_time, status, created_by_user_id
      ) VALUES (?, ?, ?, ?, ?, ?, ?, 'Draft', ?)
    `, [newSerialNumber, site_id, permit_type, work_description, work_location, start_time, end_time, req.user.id]);
    ...
    if (team_members && team_members.length > 0) {
      for (const member of team_members) {
        await connection.query(`
          INSERT INTO permit_team_members (permit_id, worker_name, worker_id, role)
          VALUES (?, ?, ?, ?)
        `, [permitId, member.worker_name, member.worker_id, member.role]);
      }
    }
    ...
    if (hazards && hazards.length > 0) {
      for (const hazardId of hazards) {
        await connection.query(`
          INSERT INTO permit_hazards (permit_id, hazard_id)
          VALUES (?, ?)
        `, [permitId, hazardId]);
      }
    }
    ...
    // Insert PPE
    if (ppe && ppe.length > 0) {
      for (const ppeId of ppe) {
        await connection.query(`
          INSERT INTO permit_ppe (permit_id, ppe_id)
          VALUES (?, ?)
        `, [permitId, ppeId]);
      }
    }
