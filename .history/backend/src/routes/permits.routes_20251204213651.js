...
// backend/src/routes/permits.routes.js - ADD THESE ROUTES
...
router.post('/:id/final-submit', async (req, res) => {
  ...
  try {
    ...
    console.log('📝 Final submit request for PTW:', id);
    ...
    if (permit.created_by_user_id !== userId) {
      ...
      return res.status(403).json({
        success: false,
        message: 'Only the PTW creator can do final submit'
      });
    }
    ...
    if (permit.status !== 'Approved') {
      ...
      return res.status(400).json({
        success: false,
        message: `Cannot final submit PTW with status: ${permit.status}. Must be Approved.`
      });
    }

    // Verify all assigned approvers have approved
    let allApproved = true;
    if (permit.area_manager_id && permit.area_manager_status !== 'Approved') allApproved = false;
    if (permit.safety_officer_id && permit.safety_officer_status !== 'Approved') allApproved = false;
    if (permit.site_leader_id && permit.site_leader_status !== 'Approved') allApproved = false;

    if (!allApproved) {
      await connection.rollback();
      return res.status(400).json({
        success: false,
        message: 'All approvers must approve before final submit'
      });
    }
    ...
    console.log(`✅ PTW ${id} final submitted → Ready_To_Start`);

    await connection.commit();

    res.json({
      success: true,
      message: 'PTW final submitted successfully. It is now ready to start.',
      data: {
        permit_id: id,
    ...
    res.status(500).json({
      success: false,
      message: 'Error final submitting PTW',
      error: error.message
    });
  } finally {
    connection.release();
...
// POST /api/permits/:id/start - Supervisor starts PTW (work begins)
router.post('/:id/start', async (req, res) => {
  ...
  try {
    ...
    console.log('▶️ Start PTW request:', id);
    ...
    if (permit.created_by_user_id !== userId) {
      ...
      return res.status(403).json({
        success: false,
        message: 'Only the PTW creator can start the PTW'
      });
    }
    ...
    if (permit.status !== 'Ready_To_Start') {
      ...
      return res.status(400).json({
        success: false,
        message: `Cannot start PTW with status: ${permit.status}. Must be Ready_To_Start.`
      });
    }
    ...
    if (now < startTime) {
      ...
      return res.status(400).json({
        success: false,
        message: `PTW can only be started after ${startTime.toLocaleString()}. Current time: ${now.toLocaleString()}`
      });
    }

    // Update status to Active (In Progress)
    await connection.query(
      `UPDATE permits 
       SET status = 'Active',
           started_at = NOW(),
    ...
    console.log(`✅ PTW ${id} started → Active (In Progress)`);

    await connection.commit();

    res.json({
      success: true,
      message: 'PTW started successfully. Work is now in progress.',
      data: {
        permit_id: id,
        status: 'Active',
        started_at: new Date()
...
// GET /api/permits/my-initiated - Get PTWs I created that are initiated (pending approval)
...
// GET /api/permits/my-approved - Get PTWs I created that are approved (waiting for final submit)
...
// GET /api/permits/my-ready-to-start - Get PTWs ready to start
...
// GET /api/permits/my-in-progress - Get PTWs in progress (Active)
