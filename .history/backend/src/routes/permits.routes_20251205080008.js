...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    // ADD THIS LOGGING
    console.log('🔍 permit_types received:', permit_types);
    console.log('🔍 permit_types type:', typeof permit_types);
    console.log('🔍 permit_types is array:', Array.isArray(permit_types));
    ...
    // ADD PERMIT_TYPES VALIDATION
    if (!permit_types || (Array.isArray(permit_types) && permit_types.length === 0)) {
      await connection.rollback();
      return res.status(400).json({
        success: false,
    ...
    // FIX PERMIT_TYPE PROCESSING
    let permitTypeString;
    if (Array.isArray(permit_types)) {
      permitTypeString = permit_types.join(',');
    } else if (typeof permit_types === 'string') {
      permitTypeString = permit_types;
    } else {
      permitTypeString = 'General'; // Default fallback
    }

    console.log('✅ Processing permit_type as:', permitTypeString);
    console.log(`✅ Creating PTW: ${permit_serial}, Status: ${initialStatus}`);

    // Insert permit - USE permitTypeString
    ...
    `, [
      permit_serial,
      site_id,
      permitTypeString,  // CHANGED FROM: Array.isArray(permit_types) ? permit_types.join(',') : permit_types
      work_description,
      work_location,
      start_time,
      end_time,
