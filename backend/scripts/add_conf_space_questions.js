...
async function addQuestions() {
    ...
    try {
        ...
        const questions = [
            { text: 'Entrant Name', type: 'text' },
            { text: 'Attendant Name', type: 'text' },
        ...
        console.log('Adding Confined Space checklist questions...');

        // Get the current max display_order
        const [rows] = await connection.query(`
            SELECT MAX(display_order) as maxOrder 
            FROM master_checklist_questions 
            WHERE permit_type = 'Confined_Space'
        `);
        ...
        for (const q of questions) {
            ...
            const [existing] = await connection.query(`
                ...
                WHERE permit_type = 'Confined_Space' AND question_text = ?
            `, [q.text]);
            ...
            if (existing.length === 0) {
                await connection.query(`
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                `, ['Confined_Space', 'Personnel Requirements', q.text, q.type, true, initialOrder++, true]);
